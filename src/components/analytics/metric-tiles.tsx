 ...
 */
export function MetricTiles({ items, columns = 4 }: { items: MetricTileItem[]; columns?: 2 | 3 | 4 | 5 | 6 }) {
  const colClass =
    columns === 2
      ? "sm:grid-cols-2"
      : columns === 3
    ...
    <div className={`grid grid-cols-2 gap-3 ${colClass}`}>
      {items.map((item) => (
        <div key={item.key} className="rounded-[var(--radius-sm)] border border-border bg-card px-3.5 py-3">
          ...
          {item.tier && item.tier !== "live" && <TierBadge tier={item.tier} label={item.tierLabel} className="mt-1.5" />}
        </div>
      ))}
    </div>
  )
